...
- [x] G7: cores institucionais (azul, verde, areia da marca) presentes nos tokens de tema
  EVIDENCE: web/src/app/globals.css define --color-navy:#1f4e68, --color-forest:#3e6b42, --color-sand:#c99a66 (aprofundamento editorial das cores oficiais do manual #226185/#5C925C/#D1AE86 -- ver docs/design-tokens.md v2 para a justificativa da redução de luminosidade e novo papel de fundo).
...
- [x] G11: interacoes de motion (entrada do hero, cta magnetico) nao quebram layout e permanecem utilizaveis sem JS ou com prefers-reduced-motion
...
- [x] G13: Lighthouse mobile: performance >= 85, acessibilidade >= 90
  EVIDENCE: rodado com sucesso usando o Chromium ja baixado pelo Playwright como executavel do Lighthouse (`npx lighthouse` + `chrome-launcher`, instalados com --no-save), contra servidor de producao real (`npm run build && npm run start`), mobile 390x844, throttling simulado. Primeira rodada: performance 96, acessibilidade 88 (abaixo da meta), best-practices 92, seo 100 -- a causa da acessibilidade baixa foi uma auditoria "list/listitem" real (ver G18). Apos corrigir, segunda rodada: performance 92, acessibilidade 96, best-practices 100, seo 100. Apos a reformulacao editorial completa (G19, troca de fontes para Fraunces+Public Sans inclusa), terceira rodada: performance 91, acessibilidade 100, best-practices 100, seo 100. Todas as metas seguem atingidas.

- [x] G18: elementos de lista semantica (<ul>/<li>) nao tem elemento nao-list-item entre pai e filho
  CHECK: node -e "const fs=require('fs'); const out=fs.readFileSync('web/src/components/Included.tsx','utf8'); console.log(out.includes('<li') && !out.includes('SectionReveal') ? 'CLEAN' : 'MISSING')"
  EXPECT: CLEAN
  EVIDENCE: Lighthouse (G13) encontrou list/listitem quebrados -- Included.tsx tinha `<SectionReveal><li>...</li></SectionReveal>` dentro de `<ul>`, e o SectionReveal renderiza um `<div>` por padrao, quebrando a relacao pai-filho semantica exigida (ul > li direto). Corrigido originalmente com prop `as="li"` no SectionReveal; na reformulacao editorial (G19) o scroll-reveal por secao foi removido inteiramente, entao Included.tsx hoje usa `<li>` puro direto dentro do `<ul>`, sem wrapper nenhum -- solucao mais simples que resolve a mesma auditoria.
...
- [x] G17: prova social tem espaco estrutural para numeros concretos reais, sem fabricar dado nao verificado
  EVIDENCE: recomendacao 3 de docs/reviews/testador-e-juiz.md (juiz apontou prova social como ponto mais fraco: 6/10). siteConfig.stats (web/src/lib/site-config.ts) segue o mesmo padrao de siteConfig.testimonials -- vazio por padrao. Na reformulacao editorial (G19) SocialProof.tsx foi fundido em FinalCta.tsx (a citacao de credibilidade e a secao final viraram uma unica faixa navy continua, evitando o vazio visual de duas secoes navy separadas); os blocos de stats/depoimentos continuam so renderizando com conteudo real. Nenhum numero de anos de atuacao ou atendimentos foi inventado.

- [x] G19: pagina nao segue o padrao generico "SaaS card kit" (cards identicos, sombra soft, fade-in por secao, hover 3D) sinalizado como design gerado por IA
  EVIDENCE: cliente rejeitou a v1 explicitamente por parecer generica ("nao usou nem 1% da capacidade"). Reformulacao completa seguindo a skill frontend-design: paleta editorial ancorada na marca real (nao a combinacao clichê creme+terracota), tipografia com personalidade (serif Fraunces + Public Sans, nao Inter generico), layout especifico por secao (lista editorial, linha do tempo vertical, lista tipografica em duas colunas, faixa de citacao) em vez de grid de cards identicos, motivo grafico original (HomeMotif.tsx) em vez de placeholder de foto de banco de imagens, um unico momento de motion orquestrado (entrada do hero) em vez de fade-in repetido em cada secao. TiltCard.tsx e motion/section-reveal.tsx removidos do codebase por serem o efeito generico. npm run build e npm run lint permanecem verdes apos a reformulacao completa.
